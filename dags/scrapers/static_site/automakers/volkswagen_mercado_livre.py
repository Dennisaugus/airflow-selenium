...
class VolkswagenMercadoLivre(MercadoLivre):
    """
    VolkswagenMercadoLivre is a child class built from the abstract class (MercadoLivre)
    ...
    def get_product_code(self, store_product, product_name, product_link):
        ...
        for word in product_name.split():
            if any(map(str.isdigit, word)) and any(map(str.isalpha, word)):
        ...
        if not product_code:
            ...
            try:
                ...
                for tag in tags_span:
                    code = tag.text
                    if (
                        any(map(str.isdigit, code))
                        and any(map(str.isalpha, code))
                        and len(code) in self.possible_length
                    ):
                        product_code = code.translate(
