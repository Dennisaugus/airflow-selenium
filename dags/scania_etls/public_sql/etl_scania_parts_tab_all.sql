-- Insert na tabela [scania_parts_tab] - atualiza todos os meses
...
SELECT
    ...
    COALESCE(LAG(ts.percentage_in_revenue, 1, NULL) OVER (PARTITION BY ts.part_number ORDER BY ts.month_year RANGE BETWEEN interval '1 month' PRECEDING AND CURRENT ROW), 0),
    ...
    ts.demand,
    (pl.rrp_real * 2.9242) AS rrp_real, -- esse 2 representa o valor do dolar no mes
...
LEFT JOIN view_maintenance_percentage vmp
    ON pl.part_number = vmp.part_number
    AND pl.month_year = vmp.month_year
LEFT JOIN percentage_in_profit_calc pipc
    ON pl.part_number = pipc.part_number
    AND pl.month_year = pipc.month_year
ON CONFLICT ON CONSTRAINT unique_part_number_insertion_date
DO UPDATE
SET percentage_in_revenue = EXCLUDED.percentage_in_revenue,
    demand = EXCLUDED.demand,
    sales_unit_price = EXCLUDED.sales_unit_price,
    revenue = EXCLUDED.revenue,
    revenue_diff_last_month = EXCLUDED.revenue_diff_last_month,
    demand_last_month = EXCLUDED.demand_last_month,
    ...
    sm_percentage_in_profit = EXCLUDED.sm_percentage_in_profit,
    bu_percentage_in_profit = EXCLUDED.bu_percentage_in_profit;
