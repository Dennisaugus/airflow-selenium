...
def get_last_month_year_from_db(connection, table_name: str) -> str:
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                f"""
                SELECT TO_CHAR(max(insertion_date), 'YYYY-MM-DD') AS max_insertion_date
...
def main(conn_id):
    ...
    max_table_date = get_last_month_year_from_db(connection, "monthly_demand_forecast")
    ...
    if latest_file_date > max_table_date:
        ...
        df = pd.read_csv(
            ...
            dtype={
                "part_number": str,
                "demand_best_price": float,
                "demand_retail_price": float,
            },
        )
        ...
        try:
            ...
            load_to_db(connection, df, "public.monthly_demand_forecast")
